...
import com.openworld.app.ui.components.*
import com.openworld.app.ui.theme.Neutral500
import com.openworld.app.ui.theme.Neutral700
...
fun AppRuleItem(
    ...
    val appIcon = remember(rule.packageName) {
        try {
            context.packageManager.getApplicationIcon(rule.packageName).toBitmap(128, 128).asImageBitmap()
        } catch (e: Exception) {
            null
        }
    }
        ...
        Row(
            ...
            if (appIcon != null) {
                ...
                Box(
                    ...
                    Icon(Icons.Rounded.Apps, contentDescription = null, tint = Neutral500)
                }
            }
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Icon(outboundIcon, contentDescription = null, tint = color, modifier = Modifier.size(14.dp))
            ...
            IconButton(onClick = onDelete) {
                Icon(Icons.Rounded.Delete, contentDescription = stringResource(R.string.common_delete), tint = MaterialTheme.colorScheme.error)
            }
        }
    }
}
...
fun AppRuleEditorDialog(
    ...
    var selectedPackageName by remember { mutableStateOf(initialRule?.packageName ?: "") }
    var appName by remember { mutableStateOf(initialRule?.appName ?: "") }
    ...
    if (showAppSelector) {
        val alreadySelected = if (selectedPackageName.isNotEmpty()) setOf(AppInfo(selectedPackageName, appName)) else emptySet()
        MultiAppSelectorDialog(
            installedApps = installedApps,
            selectedApps = alreadySelected,
            onConfirm = { apps ->
    ...
    AlertDialog(
        ...
        title = {
            Text(
                text = if (initialRule == null) stringResource(R.string.app_rules_add) else stringResource(R.string.app_rules_edit),
                fontWeight = FontWeight.Bold,
                color = MaterialTheme.colorScheme.onSurface
            )
        },
        text = {
            Column(
                ...
                ClickableDropdownField(
                    label = stringResource(R.string.app_rules_app),
                    value = if (selectedPackageName.isNotEmpty()) "$appName ($selectedPackageName)" else stringResource(R.string.app_rules_select_app),
...
fun AppGroupItem(
    ...
    AppGroupCard(group, outboundText, onClick, onToggle, onDelete)
}
