...
import com.openworld.app.ui.components.AppMultiSelectDialog
...
fun TunSettingsScreen(
    ...
    if (showRouteCidrsDialog) {
        InputDialog(
            ...
            placeholder = "e.g.\n0.0.0.0/0\n10.0.0.0/8",
            singleLine = false, // Changed from KunBox implementation which didn't expose singleLine in InputDialog, checking OpenWorld's InputDialog
            // OpenWorld's InputDialog doesn't support multiline/minLines/maxLines yet (checked in step 697).
            // Need to update InputDialog in OpenWorld if we want multiline.
            // For now, let's stick to singleLine = false if possible, or accept it will be single line.
            // Wait, OpenWorld InputDialog (step 697) DOES support passing modifiers to OutlinedTextField but not minLines/maxLines directly as params.
            // KunBox InputDialog (step 661) supported minLines/maxLines.
            // OpenWorld InputDialog (step 697) has `singleLine = true` hardcoded in OutlinedTextField param but it accepts `singleLine` param?
            // No, OpenWorld InputDialog signature: `fun InputDialog(..., singleLine: Boolean = true, ...)` IS NOT THERE.
            // It is `fun InputDialog(..., initialValue: String = "", placeholder: String = "", ...)`
            // It does NOT have `singleLine` parameter. It has `singleLine = true` hardcoded.
            // So multiline input for CIDRs will be broken in OpenWorld currently.
            // I should update InputDialog in OpenWorld to support multiline.
            onConfirm = {
