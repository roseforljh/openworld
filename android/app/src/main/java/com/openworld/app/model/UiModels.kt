...
data class BatchUpdateResult(
    ...
    fun toDisplayMessage(context: Context): String {
        return when {
             // R.string resources might not exist yet, need to check or stub
            totalCount == 0 -> "没有可更新的订阅"
            failed == totalCount -> "所有订阅更新失败"
            successWithChanges > 0 && failed == 0 -> "更新成功: ${successWithChanges} 个订阅有更新"
            successNoChanges == totalCount -> "所有订阅已是最新"
            failed > 0 -> "部分更新: ${successCount} 成功, ${failed} 失败"
            else -> "更新完成"
        }
    }
}
